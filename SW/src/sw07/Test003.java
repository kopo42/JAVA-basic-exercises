...
public class Test003 {
	...
	public static String numstr (String k42_str) { //숫자를 한글로 읽어주는 함수
		// str = 35.1067043 <- 바꿀 숫자의 형태
		k42_str = k42_str.replace(".", ""); //"." 제거
		String k42_strVoice = ""; //형식을 바꾸어 반환할 문자열
		
		String[] k42_units = {"영", "일", "이", "삼", "사", "오", "육", "칠", "팔", "구"}; //한글형을 배열에 저장하고 사용한다
		String[] k42_unitX = {"", "십", "백", "천", "만", "십", "백", "천", "억", "십"};
		int k42_i = 0;
		int k42_j = k42_str.length() - 1; //뒤에서부터 세는 기능을 수행할 정수형 변수
		
		while(true) { //무한 반복
			if(k42_i >= k42_str.length()) {
				break;
			}
			if(k42_str.substring(k42_i, k42_i+1).equals("0")) { //해당 자리가 0일 때는 단위값을 붙이지 않는다
				if(k42_unitX[k42_j].equals("만") | k42_unitX[k42_j].equals("억")) { //단 억, 만 자리에는 붙임
					k42_strVoice = k42_strVoice + "" + k42_unitX[k42_j];
				} else { //아닐 경우 시행할 동작 없음
				}
			} else {
				k42_strVoice = k42_strVoice + k42_units[Integer.parseInt(k42_str.substring(k42_i, k42_i+1))] //한글형으로 변환하여 문자열에 저장한다
						+ k42_unitX[k42_j];
			}
			k42_i++; k42_j--; //다음 항으로 넘어가기 위해 더한다
		}
		return k42_strVoice; //변환하여 반환할 문자열
	}
}
